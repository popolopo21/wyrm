...
<T.DirectionalLight position={[20, 3, 200]} intensity={4} color="#fff" />
...
<Align auto position.y={100}>
	{#each articles as domain, i}
		<T.Mesh position={[-20,0, -9+(18 * i)]} rotation={[-1.5,0,0]}>
			<Text text={
				String(domain.domain).replace("https://", "")}
				fontSize={18}
				anchorX="100%"
				anchorY="10%"
			/>
		...
		{#each domain.articles as day, j}
			{#if day}
				<T.Group position={[0, 0, 18 * i]}>
					<T.Mesh position={[18 * j, day.elements.length / 2, 0]}
					scale={$scale}
					on:pointerenter={()=> scale.set(1.5)}
					on:pointerleave={()=> scale.set(1)}
					on:click={() =>{console.log(day.key.day)}}>
						<T.BoxGeometry args={[10, day.elements.length, 10]} />
						<T.MeshStandardMaterial color="orange" />
					</T.Mesh>
					{#if i===0}
						<T.Mesh position={[(18 * j)+2,0, 45]} rotation={[-1.5,0,1.2]}>
							<Text text={
								String(day.key.day)}
								fontSize={9}
								anchorX="100%"
