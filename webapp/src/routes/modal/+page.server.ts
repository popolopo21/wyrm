...
function getd3data(data: GetArticlesReturns, startDate: Date, endDate: Date): ChartData[] {
  ...
  while (currentDate <= endDate) {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      let currentHourData = data.find(element => parseDate(element.key.hour).getTime() === currentDate.getTime());
...
export const load = (async () => {
    const data = await getArticles(client, {domain:domain, start_date:startDate, end_date:endDate});
